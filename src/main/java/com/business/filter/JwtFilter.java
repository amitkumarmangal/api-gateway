...
public class JwtFilter implements WebFilter {
    ...
    private static final List<String> BYPASS_URL= List.of("/actuator","/h2-console","/favicon.ico");
    private static final List<String> ALLOWED_USERS = List.of("admin", "bankuser");

    @Override
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        if(BYPASS_URL.stream().noneMatch(url -> exchange.getRequest().getURI().getPath().contains(url))) {
            try {
